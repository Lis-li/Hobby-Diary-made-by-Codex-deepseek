...
function normalizeHobby(h) {
  return {
    ...
    metric: (h.metric === 'weight' || h.metric === 'bloodSugar' || h.metric === 'bloodPressure') ? h.metric : null,
...
function normalizeRecord(r) {
  return {
    ...
    p1: (r.p1 !== null && r.p1 !== undefined && r.p1 !== '') ? Number(r.p1) : null,
    p2: (r.p2 !== null && r.p2 !== undefined && r.p2 !== '') ? Number(r.p2) : null,
    p3: (r.p3 !== null && r.p3 !== undefined && r.p3 !== '') ? Number(r.p3) : null,
...
function renderHealthChartHtml() {
  ...
  if (healthMetric === 'bloodPressure') {
    ...
    series = [1, 2, 3].map(i => ({
      label: labels[i - 1],
      color: colors[i - 1],
      pts: recs.filter(r => r['p' + i] != null).map(r => ({ date: r.date, v: r['p' + i] }))
    }));
  }
  ...
  const dots = series.map(s => s.pts.map(p => {
    const tip = `${s.label} ${p.v} · ${fmtShortDate(p.date)}`;
  ...
  const axis = `
    ...
    <text x="${padL - 6}" y="${y(max) + 4}" text-anchor="end" class="axis-label">${max}</text>
    <text x="${padL - 6}" y="${y(min) + 4}" text-anchor="end" class="axis-label">${min}</text>
...
function openRecordModal(date, record, preferredHobbyId, categoryFilter) {
  ...
  if (record) {
    ...
    } else if (record.metric === 'weight' || record.metric === 'bloodSugar') {
      [record.p1, record.p2, record.p3].forEach((v, i) => {
        const inp = $('#health-p' + (i + 1));
...
function currentModalHobbyMetric() {
  const sel = $('#record-form [name="hobbyId"]');
  const h = sel ? hobbyById(sel.value) : null;
  if (h && (h.category || 'hobby') === 'health') return h.metric || 'weight';
  return null;
}
...
async function onRecordSubmit(e) {
  ...
  if (cat === 'health') {
    metric = currentModalHobbyMetric() || 'weight';
  ...
  // 健康按天合并：同一天同一项目已有一条记录时，只补填本次填写的组
  if (cat === 'health' && (metric === 'weight' || metric === 'bloodSugar') && !modalRecordId) {
    const existing = recordFor(date, hobbyId);
    if (existing) {
      if (p1 !== null) existing.p1 = p1;
      if (p2 !== null) existing.p2 = p2;
      if (p3 !== null) existing.p3 = p3;
      existing.metric = metric;
      if (note && !existing.note) existing.note = note;
      if (selectedMood !== null && existing.mood === null) existing.mood = selectedMood;
      if (photos.length && (!existing.photos || !existing.photos.length)) existing.photos = photos;
      saveState();
      closeModal();
      renderAll();
      toast(T('已补录'));
...
function openHobbyModal(hobby, presetCategory) {
  modalHobbyId = hobby ? hobby.id : null;
  selectedColor = hobby ? hobby.color : COLOR_PRESETS[0];
  selectedProjectCategory = hobby ? hobby.category : (presetCategory || 'hobby');
  selectedProjectMetric = hobby && hobby.metric ? hobby.metric : 'weight';
...
function onAction(action, el) {
  switch (action) {
    ...
    case 'open-record': {
      ...
      openRecordModal(currentDate, rec, el.dataset.hobby, todayCategory);
      break;
    }
    ...
    case 'edit-record': {
      ...
      else openRecordModal(date, null, el.dataset.hobby, el.dataset.date ? null : todayCategory);
      break;
    }
    case 'add-record': openRecordModal(el.dataset.date || currentDate, null, null, el.dataset.date ? null : todayCategory); break;
    ...
    case 'toggle-project-group': {
      if (Object.prototype.hasOwnProperty.call(projectGroupsOpen, el.dataset.group)) {
        projectGroupsOpen[el.dataset.group] = !projectGroupsOpen[el.dataset.group];
        renderHobbies();
      }
      break;
    }
