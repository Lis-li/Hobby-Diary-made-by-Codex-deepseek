...
- localStorage 键：`hobby-diary:v1`（数据）、`hobby-diary:theme`（主题）、`hobby-diary:focus-session`（专注计时会话）、`hobby-diary:lock`（应用锁密码哈希，仅本机保存）、`hobby-diary:music-style`（背景音乐风格）、`hobby-diary:health-seeded`（健康内置项目是否已添加）
